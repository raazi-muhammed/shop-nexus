...
const WishListUser = () => {
    ...
    const handleAddToCart = (productId, discountPrice) => {
        ...
        axios
            .post(`${server}/cart/add-to-cart`, itemData, {
    ...
    useEffect(() => {
        ...
        axios
            .get(`${server}/wish-list/get-all-wish-list`, {
                withCredentials: true,
            })
            .then((res) => {
                dispatch(setUserDataReducer(res.data?.user));
            ...
            .catch((err) =>
                toast.error(
                    err?.response?.data?.message || "Error Loading Wishlist"
                )
            );
    }, [isWishListVisible]);
            ...
            >
                <section className="offcanvas-body pt-5 w-100">
                    ...
                    {wishListItems?.map((wishListItem, i) => (
                        <div
                            key={i}
                            className="row w-100 m-0 my-2 align-items-center"
                        >
